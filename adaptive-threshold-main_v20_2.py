...
IMAGE_NUM_LIMIT = 2
OUTPUT_CSV_PATH = 'data/results_csv_v20_2'
...
def find_threshold_stats(mydirs):
    ...
    for dir_name in mydirs:
        if len(list(f for f in glob.glob(os.path.join(SRC_PATH, dir_name)+'/*'))) > IMAGE_NUM_LIMIT:
            cropped_faces = list()
...
def main():
    ...
    updated_dirs = list()
    directories = [mydir for mydir in os.listdir(SRC_PATH)]
    for d in directories:
        if len(list(f for f in glob.glob(os.path.join(SRC_PATH, d)+'/*'))) > IMAGE_NUM_LIMIT:
    ...
    for d in updated_dirs:
        ...
        if dir_count < 1:
            ...
            for mydir in mydirs:
                ...
                for img in glob.glob(os.path.join(SRC_PATH, mydir)+'/*'):
                ...
                if len(query_embeddings) is not 0:
    ...
    print('Percentage of precision@adaptive_threshold >= 0.9: {}%'.format(pcount/(ptotal+1e-7)*100))
    print('Percentage of precision@fixed_threshold=0.3 >= 0.9: {}%'.format(pfixed_count_1/(ptotal+1e-7)*100))
    ...
    mymodel = np.poly1d(np.polyfit(FPRs_adaptive, TPRs_adaptive, 2))
    ...
    ax4.plot([0,1], [0,1], linestyle='dashed', linewidth=3, color='black')
    ...
    ax7.plot(people_count, fixed_precisions_1, label='fixed_threshold=0.3', linewidth=3, color='darkorange')
    ax7.plot(people_count, fixed_precisions_2, label='fixed_threshold=0.5', linewidth=3, color='green')
    ax7.plot(people_count, fixed_precisions_3, label='fixed_threshold=0.7', linewidth=3, color='red')
    ...
    ax8.plot(people_count, fixed_recalls_1, label='fixed_threshold=0.3', linewidth=3, color='darkorange')
    ax8.plot(people_count, fixed_recalls_2, label='fixed_threshold=0.5', linewidth=3, color='green')
    ax8.plot(people_count, fixed_recalls_3, label='fixed_threshold=0.7', linewidth=3, color='red')
    ...
    ax9.plot(people_count, fixed_f1scores_1, label='fixed_threshold=0.3', linewidth=3, color='darkorange')
    ax9.plot(people_count, fixed_f1scores_2, label='fixed_threshold=0.5', linewidth=3, color='green')
    ax9.plot(people_count, fixed_f1scores_3, label='fixed_threshold=0.7', linewidth=3, color='red')
