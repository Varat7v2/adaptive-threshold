...
def main():
    ...
    selected_folder= random.sample(set(directories), int(directories_len*myconfig.FOLDER_PERCENT))
    print('No. of folders chosen: ', len(selected_folder))
    print(selected_folder)

    for d in selected_folder:
        img_lst = list()
        for f in glob.glob(os.path.join(myconfig.SRC_PATH, d)+'/*'):
            img_lst.append(f)

        img_count = len(img_lst)
        random_img = random.sample(set(img_lst), int(img_count*myconfig.IMAGE_PERCENT))
        updated_dict[d] = list(random_img)

    
    # for d in directories:
        # if len(list(f for f in glob.glob(os.path.join(myconfig.SRC_PATH, d)+'/*'))) >= myconfig.IMAGE_NUM_LIMIT:
        #     updated_dirs.append(d)                                                                   ----------commented zainab
    ...
    # for d in updated_dict:
    #     loop_count += 1
    #     if loop_count == len(updated_dict):
    #         PLOT_FLAG = True
    #     num_people += 1
    #     if dir_count < 1:
    #         mydirs.append(d)
